...
- v2 云处理缩略图：通过 `?w={宽度}` 参数生成 Halo 的 S/M/L/XL（400/800/1200/1600）四档缩略图
- v2 上传参数扩展：支持去 EXIF（隐私）、设为公开图片
- v2 按日期自动归档：未指定相册时，按「yyyy-MM」（如 2026-09）自动创建或复用相册
...
- 缩略图仅 v2 商业版支持，且需在兰空后台开启「云处理 → 支持参数 `w`」（默认关闭，官方提示随意开启易被刷图攻击）。v1 开源版仍不支持缩略图（其仅能生成单个预定义尺寸，不满足 Halo 多尺寸要求）。
